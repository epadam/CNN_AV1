...
sample_file =  'ash_samples_64_intra.txt'
label_file = 'ash_labels_64_intra.txt'
qp_file = 'ash_qps_64_intra.txt'
#read samples
...
print('labels', np.shape(single_label))  
...
print('qps', np.shape(qps)) 

f.close()
f_single_label.close()
f_qp.close()

label_None = np.zeros(12000)
label_H = np.zeros(2500)
label_V = np.zeros(2500)
label_S = np.zeros(60000)
label_HA = np.zeros(1000)
label_HB = np.zeros(1000)
label_VA = np.zeros(1000)
label_VB = np.zeros(1000)
label_H4 = np.zeros(2000)
label_V4 = np.zeros(2000)



qp_None = np.zeros((12000,1))
qp_H = np.zeros((2500,1))
qp_V = np.zeros((2500,1))
qp_S = np.zeros((60000,1))
qp_HA = np.zeros((1000,1))
qp_HB = np.zeros((1000,1))
qp_VA = np.zeros((1000,1))
qp_VB = np.zeros((1000,1))
qp_H4 = np.zeros((2000,1))
qp_V4 = np.zeros((2000,1))

data_None = np.zeros([12000,block_size, block_size, NUM_CHANNELS])
data_H = np.zeros([2500,block_size, block_size, NUM_CHANNELS])
data_V = np.zeros((2500,block_size, block_size, NUM_CHANNELS))
data_S = np.zeros((60000,block_size, block_size, NUM_CHANNELS))
data_HA = np.zeros((1000,block_size, block_size, NUM_CHANNELS))
data_HB = np.zeros((1000,block_size, block_size, NUM_CHANNELS))
data_VA = np.zeros((1000,block_size, block_size, NUM_CHANNELS))
data_VB = np.zeros((1000,block_size, block_size, NUM_CHANNELS))
data_H4 = np.zeros((2000,block_size, block_size, NUM_CHANNELS))
data_V4 = np.zeros((2000,block_size, block_size, NUM_CHANNELS))
...
plt.ylim(top=100)
plt.title('11 frames, block size=64, qp=120, total smaples='+str(total), fontsize=10)
for i in range(len(K)):
    plt.text(x=Index[i]-0.2, y=perc[i]+0.7, s = perc[i], fontsize=8)
plt.savefig(foldername +'_distribution_64.jpg')
plt.show
...
shuffle_sample_file = foldername + 'anoS_samples_64_intra.txt'
shuffle_label_file = foldername + 'anoS_labels_64_intra.txt'
shuffle_qp_file = foldername + 'anoS_qps_64_intra.txt'
