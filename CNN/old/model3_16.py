...
num_conv2_concat = 1 * 1 * num_conv2_filter 
...
def net(x, qp, drop):
    ...
    conv1 = non_overlap_conv(image_m, 8, NUM_CHANNELS, num_conv1_filter, acti_mode = 1, name='conv1') #out batch*2*2*16
    conv2 = non_overlap_conv(conv1, 2, num_conv1_filter, num_conv2_filter, acti_mode = 1, name='conv2') # out batch*1*1*24
   
    conv2_flat = tf.reshape(conv2, [-1, num_conv2_concat])
    conv2_flat_c = tf.concat([conv2_flat, qp], axis=1)

    fc_1 = full_connect(conv2_flat_c, num_conv2_concat + num_extra, num_full_con, acti_mode=1, rate=drop*0.5, name='fc1')
    fc_1_c = tf.concat([fc_1, qp], axis=1)

    prob = full_connect(fc_1_c, num_full_con + num_extra, num_label, acti_mode = 0 , name='output')
