...
def non_overlap_conv(x, k_width, num_filters_in, num_filters_out, acti_mode, name=None):
...
def full_connect(x, num_filters_in, num_filters_out, acti_mode, rate=0, name=None): 
    with tf.name_scope(name):
...
def net(x, qp, drop):
    ...
    with tf.name_scope('input_reshape'):
        ...
        image_m = zero_mean_norm_local(aver_pool(x_image, 4), 16, 16)
        tf.summary.image('input', image_m, 10)

    conv1 = non_overlap_conv(image_m, 4, NUM_CHANNELS, num_conv1_filter, acti_mode = 1, name='conv1') #out batch*4*4*16
    conv2 = non_overlap_conv(conv1, 2, num_conv1_filter, num_conv2_filter, acti_mode = 1, name='conv2') # out batch*2*2*24
    conv3 = non_overlap_conv(conv2, 2, num_conv2_filter, num_conv3_filter, acti_mode = 1, name='conv3') # out batch*1*1*32
    ...
    fc_1 = full_connect(conv_flat_all, num_concat_all, num_full_con1, acti_mode=1, rate=drop*0.5, name='fc1')
    fc_1_c = tf.concat([fc_1, qp], axis=1)

    fc_2 = full_connect(fc_1_c, num_full_con1 + num_extra, num_full_con2, acti_mode=1, rate=drop*0.2, name='fc2')
    fc_2_c = tf.concat([fc_2, qp], axis=1)


    prob = full_connect(fc_2_c, num_full_con2 + num_extra, num_label, acti_mode = 0 , name='model_output')
...
def losses2(logits, labels):   
    with tf.variable_scope('loss') as scope:
        classes_weights = tf.constant([0.1, 10.0, 10.0, 0.01, 1.0, 10.0, 10.0, 10.0, 10.0, 10.0])
        weights = tf.reduce_sum(labels*classes_weights, axis =1)
        unweighted_loss = tf.nn.sotfmax_cross_entropy_with_logits(labels, logits)
        weighted_losses = unweighted_loss * weights
        tf.reduce_mean(weighted_losses)
        tf.summary.scalar(scope.name + '/cross_entropy', loss)  
    return loss       
...
def trainning(loss, learning_rate):  
    with tf.name_scope('training'):  #this means optimizer = tf.name_scope
        ...
        #tf.summary.scalar(scope.name + '/learning_rate', optimizer.variables(learning_rate))  
    return train_op 
