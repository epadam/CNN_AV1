...
def net(x, qp, drop):
    ...
    fc_1 = full_connect(conv3_flat_c, num_conv3_concat + num_extra, num_full_con, acti_mode=1, rate=drop*0.5, name='fc1')
    fc_1_c = tf.concat([fc_1, qp], axis=1)

    prob = full_connect(fc_1_c, num_full_con + num_extra, num_label, acti_mode = 0 , name='model_output')
...
'''
def trainning2(loss, learning_rate):  
    with tf.name_scope('training'):  #this means optimizer = tf.name_scope
        optimizer = tf.train.MomentumOptimizer(learning_rate= learning_rate, 0.9)
        grads = optimizer.compute_gradients(loss)
        for i, (g, v) in enumerate(grads):
            if g is not None:
                grads[i]=(ft.clip_by_norm(g, 100), v)
